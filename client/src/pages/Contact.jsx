...
const Contact = () => {
  ...
  return (
    <>
      <PageHeader title="સંપર્ક કરો" subtitle="બુકિંગ માટે ફોર્મ ભરો અથવા સીધા WhatsApp કરો" />

      <section className="max-w-2xl mx-auto px-4 py-10">
        {/* Quick contact cards */}
        <div className="grid grid-cols-2 gap-3 mb-8">
          <a href="https://wa.me/918799008221" target="_blank" rel="noreferrer"
            className="flex flex-col items-center gap-2 bg-[#25D366]/10 border border-[#25D366]/30 rounded-2xl py-5">
          ...
          <a href="https://www.instagram.com/pili_mahendi_nail_art_007" target="_blank" rel="noreferrer"
            className="flex flex-col items-center gap-2 bg-[#E1306C]/10 border border-[#E1306C]/30 rounded-2xl py-5">
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm p-5 sm:p-6 space-y-4 border border-[#D4AF37]/10">
            ...
            <div>
              ...
              <input name="name" value={form.name} onChange={handleChange} required
                className="w-full border border-gray-300 rounded-lg px-4 py-3 text-base focus:outline-none focus:ring-1 focus:ring-[#D4AF37]" placeholder="દા.ત. પ્રિયા પટેલ" />
            </div>

            <div>
              <label className="text-sm font-medium text-[#4A2E22] block mb-1">મોબાઈલ નંબર *</label>
              <input name="phone" type="tel" value={form.phone} onChange={handleChange} required
                className="w-full border border-gray-300 rounded-lg px-4 py-3 text-base focus:outline-none focus:ring-1 focus:ring-[#D4AF37]" placeholder="9876543210" />
            </div>
            ...
            <div>
              ...
              <select name="serviceInterested" value={form.serviceInterested} onChange={handleChange}
                className="w-full border border-gray-300 rounded-lg px-4 py-3 text-base bg-white focus:outline-none focus:ring-1 focus:ring-[#D4AF37]">
            ...
            <div>
              ...
              <input name="eventDate" type="date" value={form.eventDate} onChange={handleChange}
                className="w-full border border-gray-300 rounded-lg px-4 py-3 text-base focus:outline-none focus:ring-1 focus:ring-[#D4AF37]" />
            </div>

            <div>
              <label className="text-sm font-medium text-[#4A2E22] block mb-1">સંદેશો</label>
              <textarea name="message" value={form.message} onChange={handleChange} rows={3}
                className="w-full border border-gray-300 rounded-lg px-4 py-3 text-base focus:outline-none focus:ring-1 focus:ring-[#D4AF37]" placeholder="વધુ વિગત લખો (ઓપ્શનલ)" />
            </div>

            <button type="submit" disabled={loading}
              className="w-full bg-[#6B2E1F] text-white rounded-full py-3.5 font-semibold hover:bg-[#5a271a] transition active:scale-95">
